    ...
    huf.seekForItem(configuration, config.page, config.keywords, (response, err) => {
        ...
        if (err || response == null) {
            if (config.autoRetryOnCrash == true) {
                ...
                return huf.log(i, 'error', err);
            }
        }
        return addToCart(i, config, response);
    });
  }
}

for (i in configuration.tasks) {
  stack.push(seek(configuration.tasks[i], i))
}

async.each(stack, function(res, err) {});

function addToCart(i, config, response) {

    huf.log(i, 'success', `Found your item "${response.name}"`)
    ...
    request({
        ...
        if (err) {
            return huf.log(i, 'error', 'Error has occured while getting product information.')
        } else {
        ...
        if (itemSelectSize == undefined) {
            return huf.log(i, 'error', `Could not find item available in size ${config.size}`)
        } else {
            huf.log(i, 'success', `Found item available in size ${config.size}`)
        }
        ...
        if (isEveryStyleTheSame) {
            huf.log(i, 'warning', `Every item variant is the same, your item will be checked out in ${variants[0].style}`)
            var itemToBuy = _.where(variants, {
                style: variants[0].style,
                size: config.size
            });
            huf.log(i, 'info', `Checkout out in ${variants[0].style} in size ${config.size} - ${itemToBuy[0].id}`)
            return getSessionID(i, response.link, itemToBuy[0].id, config);
        } else {
            var itemToBuy = _.where(variants, {
                size: config.size
            });
            if (itemToBuy.indexOf(config.color) > -1) {
                ...
                return huf.log(i, 'error', 'Error occured while trying to find your item using the keywords provided.')
            }
        }
...
function getSessionID(i, link, id, config) {
    huf.log(i, 'info', 'Collecting Session ID...')
    request({
        ...
        if (err) {
            return huf.log(i, 'error', 'Error has occured while trying to pickup session id')
        }

        return atc(i, link, id, config);

    });
}

function atc(i, link, id, config) {

    huf.log(i, 'info', 'Adding to Cart...')
    huf.log(i, 'info', id)
    ...
    request({
        ...
        if (err || body === undefined) {
            return huf.log(i, 'error', 'Error occured while trying to add item to your cart.')
        } else {
            var $ = cheerio.load(body);
        }

        huf.log(i, 'success', '"' + $('.name').text() + '"' + ' added to cart')
        huf.log(i, 'success', 'Price: ' + '$' + $('.unit_price').text())

        return getCheckoutPage(i, id, config);

    });

}

function getCheckoutPage(i, id, config) {
    ...
    request({
        ...
        huf.log(i, 'info', storeID)
        var checkoutID = $('.edit_checkout').attr('action').split('checkouts/')[1];
        huf.log(i, 'info', 'Checkout ID: ' + checkoutID)
        var auth_token = $('input[name=authenticity_token]').attr('value');
        huf.log(i, 'info', 'Auth Token Value: ' + auth_token)
        return input(i, id, checkoutID, auth_token, config, storeID);

    });

}

function input(i, id, checkoutID, auth_token, config, storeID) {
    ...
    request({
        ...
        huf.log(i, 'info', 'Looking for shipping options, please wait...')
        return ship(i, id, checkoutID, storeID, $('input[name=authenticity_token]').attr('value'), config);

    });

}

function ship(i, id, checkoutID, storeID, auth_token, config) {
    ...
    request({
        ...
        if (err || body === undefined) {
            return huf.log(i, 'error', 'Error occured while trying to setup shipping options.')
        } else {
            var $ = cheerio.load(body);
            ...
            if ($('.input-radio').eq(1).attr('value') === undefined) {
                return ship(i, id, checkoutID, storeID, auth_token, config);
            } else {
                ...
                huf.log(i, 'success', 'Cheapest Shipping Option Value: ' + option_1_radio_dec_val)
                return submitShippingChoice(i, storeID, checkoutID, $('input[name=authenticity_token]').attr('value'), option_1_radio_dec_val, config);
            }
        }
    });
}

function submitShippingChoice(i, storeID, checkoutID, auth_token, shippingRateID, config) {
    // get to card page
    request({
        url: `https://checkout.shopify.com/${storeID}/checkouts/${checkoutID}`,
        followAllRedirects: true,
        ...
        if (err || body === undefined) {
            return huf.log(i, 'error', 'Error occured while trying to pay for your item.')
        } else {
            var $ = cheerio.load(body);
        }
        huf.log(i, 'success', 'Shipping option has been picked.')
        var auth_token = $('input[name=authenticity_token]').attr('value')
        ...
        return sendCCInfo(i, auth_token, storeID, checkoutID, config, payment_gateway, checkout_total_price)

    });

}

function sendCCInfo(i, auth_token, storeID, checkoutID, config, payment_gateway, checkout_total_price) {
    huf.log(i, 'success', 'Sending your CC information to Shopify\'s services...')
    request({
        ...
        if (err || body === undefined) {
            return huf.log(i, 'error', 'Error occured while trying to verify your card with Shopfiy.')
        } else {
            huf.log(i, 'success', 'Card Information verified by Shopify!')
            var cc_verify_id = JSON.parse(body).id
            return checkoutCardFinal(i, auth_token, config, storeID, checkoutID, cc_verify_id, payment_gateway, checkout_total_price)
        }
    });
}

function checkoutCardFinal(i, auth_token, config, storeID, checkoutID, cc_verify_id, payment_gateway, checkout_total_price) {
  request({
      url: `https://checkout.shopify.com/${storeID}/checkouts/${checkoutID}`,
      followAllRedirects: true,
      method: 'post',
      ...
      if (err || body === undefined) {
          ...
          huf.log(i, 'success', 'Payment Successful!')
      }
